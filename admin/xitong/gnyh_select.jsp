...
try
{
  ...
  rs = database.query(" select t_yh.*, t_bm.mc bmmc" +
                      " from t_yh_gn, t_yh, t_bm" +
                      " where t_yh_gn.yhid = t_yh.yhid" +
                      "       and t_yh.bmid = t_bm.bmid" +
                      "       and t_yh_gn.gnid = '" + gnid + "'" +
                      "       and t_yh.mc like '%" + yxyhmc + "%'");
  while (rs.next())
  {
    p = new Properties();
...
{
  (new Log()).log("error", "admin/xitong/gnyh_select.jsp：" + Global.getInstance().getTime() + "：" + e.getMessage());
}
finally
{
  database.close();
...
<html>
<head>
<title>功能用户_已选列表</title>
<link href="../../style/style.css" rel="stylesheet" type="text/css">
<script language="javascript">
function work_delete(obj)
{
  if (window.confirm("确定删除数据"))
    document.gnyh_select_frame.location.href = "gnyh_do.jsp?operate=delete&gnid=<%=gnid%>&yhid=" + obj;
}

function work_go(obj) 
{
  document.gnyh_select_form.action = "gnyh_select.jsp";
  document.gnyh_select_form.target = "_self";
  document.gnyh_select_form.cpage.value = obj;
  document.gnyh_select_form.submit();
...
<body>
<form name="gnyh_select_form" method="post">
...
<iframe name="gnyh_select_frame" src="" frameborder="0" scrolling="no" width="0" height="0"></iframe>
