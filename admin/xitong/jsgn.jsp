...
{
  (new Log()).log("error", "admin/xitong/jsgn.jsp：" + Global.getInstance().getTime() + "：" + e.getMessage());
}
finally
{
  database.close();
...
{
  document.jsgn_yx_form.action = "jsgn_select.jsp?jsid=<%=jsid%>";
  document.jsgn_yx_form.submit();
}

function work_query_wx()
{
  document.jsgn_wx_form.action = "jsgn_unselect.jsp?jsid=<%=jsid%>";
  document.jsgn_wx_form.submit();
}
...
<table width="95%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td width="48%" height="340" class="text">
      <form name="jsgn_yx_form" method="post" target="jsgn_yx_frame">
      &nbsp;已选功能：<input type="text" name="yxgnmc" class="input">&nbsp;&nbsp;<input type="button" class="button" value="查询" onclick="work_query_yx();">
      </form>
      
      <br>
      
      <iframe name="jsgn_yx_frame" src="jsgn_select.jsp" frameborder="0" scrolling="no" width="100%" height="100%"></iframe>
    </td>
    <td width="4%">&nbsp;</td>
    <td width="48%" class="text">
      <form name="jsgn_wx_form" method="post" target="jsgn_wx_frame">
      &nbsp;未选功能：<input type="text" name="wxgnmc" class="input">&nbsp;&nbsp;<input type="button" class="button" value="查询" onclick="work_query_wx();">
      </form>
      
      <br>
      
      <iframe name="jsgn_wx_frame" src="jsgn_unselect.jsp" frameborder="0" scrolling="no" width="100%" height="100%"></iframe>
    </td>
  </tr>
</table>
