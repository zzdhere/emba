...
try
{
  ...
  rs = database.query("select * from t_bs where mc like '%" + bsmc + "%' order by xh");
  while (rs.next())
  {
    p = new Properties();

	 	p.setProperty("bsid", rs.getString("bsid"));
    p.setProperty("bsmc", rs.getString("mc"));
...
{
  (new Log()).log("error", "admin/xitong/bs_list.jsp：" + Global.getInstance().getTime() + "：" + e.getMessage());
}
finally
{
  database.close();
...
{
  window.open("bs_modify.jsp?list=" + obj);
}

function work_delete(obj) //删除
{
  if (window.confirm("确定删除数据"))
    document.bs_list_frame.location.href = "bs_do.jsp?operate=delete&list=" + obj;
}

function work_go(obj) 
{
  document.bs_list_form.action = "bs_list.jsp";
  document.bs_list_form.target = "_self";
  document.bs_list_form.cpage.value = obj;
  document.bs_list_form.submit();
...
<body>
<form name="bs_list_form" method="post">
      ...
      {%>
        <tr>
          ...
          <td align="center" class="td box_top box_left box_right" nowrap>
            <a href="javascript:work_modify('<%=(((Properties) v.get(i)).getProperty("bsid"))%>');"><img src="../../style/images/pencil.png" border="0" alt="修改"></a>&nbsp;
            <a href="javascript:work_delete('<%=(((Properties) v.get(i)).getProperty("bsid"))%>');"><img src="../../style/images/cross.png" border="0" alt="删除"></a>&nbsp;
          </td>
        </tr>
    <%}%>
      </table>
...
<input type="hidden" name="bsmc" value="<%=bsmc%>">
</form>

<iframe name="bs_list_frame" src="" frameborder="0" scrolling="no" width="0" height="0"></iframe>
